...
- **`wrangler dev`** runs your Worker in **workerd** (the real runtime, via Miniflare) with real
  binding semantics. Use `--local` (default; simulated D1/KV/R2 on disk) or `--remote` (runs on
  Cloudflare's edge against real resources). Local is the default and what you want most of the time.
- **Cloudflare Vite plugin** (`@cloudflare/vite-plugin`) — for Vite apps (e.g. RR7), it runs the
  app inside workerd during dev with the same bindings, so dev matches production. Prefer it over a
  Node dev server for anything that touches `env`.
- **`wrangler deploy`** uploads and activates the Worker. `wrangler deploy --env production` deploys
  a named environment. `wrangler versions upload` + gradual deployments enable preview/canary.
- **`wrangler types`** generates a typed **`Env`** interface (e.g. `worker-configuration.d.ts`) from
  your `wrangler.jsonc` bindings. Re-run it whenever you add/rename a binding. Reference the
  generated `Env` in your handler signatures (`ExportedHandler<Env>`).

```bash
wrangler types          # regenerate Env from wrangler.jsonc bindings
wrangler dev            # local workerd with simulated bindings
wrangler deploy         # deploy default env
wrangler tail           # stream live logs from the deployed Worker
```

## Other handlers

**Scheduled (cron)** — declared in `triggers.crons`, runs on schedule:
...
**Queue consumer** — co-located with `fetch`; consumer config in `queues.consumers`:

```ts
export default {
  async fetch(req, env, ctx) { /* ... producer side: env.QUEUE.send(...) */ },

  async queue(batch: MessageBatch<JobMessage>, env: Env, ctx: ExecutionContext) {
    for (const msg of batch.messages) {
      try {
        await handleJob(msg.body, env);
        msg.ack();                       // explicit ack on success
      } catch {
        msg.retry({ delaySeconds: 30 }); // redeliver later; tune retries/DLQ in config
      }
    }
  },
} satisfies ExportedHandler<Env>;
...
**Durable Objects** — a single addressable instance for coordination/state (counters, rooms,
WebSocket hubs). Declare a `durable_objects` binding + a class export + a migration tag. Reach for
them only when you need single-writer consistency or live connections; for plain relational data use
D1.

**Observability** — set `observability.enabled` in config; tail with `wrangler tail`; use
`console.log` (captured) plus structured logging.

## Limits & gotchas

- **No Node built-ins** without `nodejs_compat` — and that flag polyfills only a *subset*; many
  Node-only libraries still won't run. If a dependency imports `fs`/`net`/native addons, it likely
  won't work at the edge. Check before adopting.
- **CPU time is capped per request**; wall-clock I/O is not. Long CPU loops will be killed — move
  aggregation into SQL, stream responses.
- **No module-level state that must persist** across requests. Isolates are created/destroyed
  freely; cold starts are small but real. Keep init cheap and per-request.
- **No interactive D1 transactions** — `db.batch()` is the atomic unit; design writes around it.
- **Named-environment sections replace, not merge** — forgetting to repeat a binding in
  `env.production` is a classic "works in dev, missing in prod" bug.
- **`.dev.vars` and local D1 are separate from remote** — never assume local state mirrors prod.
- **Bump `compatibility_date` deliberately**; it changes runtime behavior. Pin it, don't float it.
